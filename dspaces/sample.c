...
void prepare_sampled_buffer(float *buffer_region, float *buffer_sample, int num_elems_region, int num_elems_sample){
    int i;
...
void assign_clusterid(float *buffer_region,int num_region, float * buffer_sample_all, int num_sample_all,float *buffer_medoids,int k,int region_length,int k_npdiv, int div_func,  float *clusterids){
    int i, j;
    float clusterid;
    float divs, divs_tmp;
    ...
    for(i = 0; i< num_region; i++){
        ...
        for( j = 0; j <k, j++){
            ...
            buffer_b = buffer_sampled_all + buffer_medoids[j]*spacing;
